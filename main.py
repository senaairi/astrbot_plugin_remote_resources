...
@register("astrbot_plugin_remote_resources", "lihz", "提供函数工具和标准接口。让AI可以读取远程系统的文件内容", "1.0.0")
class MyPlugin(Star):
        ...
        async with httpx.AsyncClient(timeout=30,verify=False) as client:
            try:
                ...
                if response.status_code != 200:
                    return CallToolResult(content=[TextContent(type="text", text=f"Error: 读取文件列表失败，http code：{response.status_code}，content：{response.text}")])
                ...
                return CallToolResult(content=[TextContent(type="text", text=f"Error: 读取远程文件列表失败，{e}")])
    ...
    async def get_user_remote_read_file(self, event: AstrMessageEvent, file_id: str) :
        ...
        async with httpx.AsyncClient(timeout=30,verify=False) as client:
            try:
                ...
                return CallToolResult(content=[TextContent(type="text", text="Error: 读取文件内容失败")])
